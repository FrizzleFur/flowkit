...
>
> **Never weaken assertions to go green（2026-08-30 补，AIBC e2e-setup 原则）**: 验证不通过时禁止通过放松成功标准、删除/跳过验证项、降低断言来"制造通过"。放松只在**契约本身确实变了**时才合法，且必须从证据确认（diff/输出），不能凭感觉。失败先分诊：真失败（修复实现）/ 过期标准（更新契约并记录）/ 环境问题（修环境）。
