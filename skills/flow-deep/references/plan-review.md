...
> 本文件由 flow / flow-deep SKILL.md 共同引用。定义 Stage 3.5 的 Plan Review 流程。
> 核心理念: "两个 Claude" 模式——用新上下文消除思维惯性。

## 概述

Plan Review 在 Stage 3（规划）完成后、Stage 3.6（多角色面板评审）之前，启动一个独立的 Agent 以 Staff Engineer 角色审查 plan。审查结果需要用户手动确认后才继续执行。
