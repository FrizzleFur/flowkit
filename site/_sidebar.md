- [首页](/)
<div class="fs-legend" style="padding:6px 15px;font-size:10.5px;color:#71717a;line-height:1.9">
域色: <span style="color:#3b82f6">●</span>编排 <span style="color:#eab308">●</span>输入 <span style="color:#8b5cf6">●</span>规划 <span style="color:#ec4899">●</span>评审 <span style="color:#22c55e">●</span>并发 <span style="color:#06b6d4">●</span>上下文 <span style="color:#ef4444">●</span>验证 <span style="color:#14b8a6">●</span>记忆 <span style="color:#f97316">●</span>自举
</div>

- [关于本站](about.md)

- **原理篇 · 地基三讲**（熟手可[直通机制篇](mechanisms/ch4-pipeline-overview.md)）

  - [1. Agent Loop 与管道形态](principles/ch1-agent-loop-and-pipeline.md)
  - [2. 长时程三板斧](principles/ch2-context-three-axes.md)
  - [3. 记忆与召回闭环](principles/ch3-memory-loop.md)

- **机制篇**

  - [4. 管道全景](mechanisms/ch4-pipeline-overview.md)
  - [5. 输入质量与思考规划](mechanisms/ch5-input-and-planning.md)
  - [6. 评审与决策](mechanisms/ch6-review-and-decision.md)
  - [7. 并发执行](mechanisms/ch7-concurrent-execution.md)
  - [8. 上下文工程](mechanisms/ch8-context-engineering.md)
  - [9. 验证与迭代](mechanisms/ch9-verification-loop.md)
  - [10. 跨会话记忆](mechanisms/ch10-cross-session-memory.md)
  - [11. 编排治理与质量自举](mechanisms/ch11-orchestration-governance.md)

- [Learning Path](path.md)

- **反哺与附录**

  - [改进提案（REC）](propositions.md)
  - [SC5 锚点复核表](anchors.md)
